...
exports.insertMunicipalitiesHasPostcode = function(){
    const query = "INSERT INTO `municipality_has_postcode` (`municipality_number`, `pct_in_municipality`, `postcode_id`) VALUES ?";
    let fieldNames;
    let PLZ4;
    let PLZZ;
    let GDENR;
    ...
    pool.getConnection(function(err, connection) {
        $QUERY = "INSERT INTO municipality_has_postcode(" +
...
exports.getAddressByPostcodeNumber = function(number) {
    ...
    return new Promise(function(resolve, reject) {
        ...
        // reject();
        pool.getConnection(function(err, connection) {
            $QUERY = "SELECT DISTINCT p.name as location, p.number as postcode, m.name as municipality_name, m.number as municipality_number FROM municipality_has_postcode mp INNER JOIN postcode p on p.number=mp.postcode_number INNER JOIN municipality m on m.number = mp.municipality_number WHERE mp.postcode_number = ?";
            if(err) { console.log('error!! '); return; }
